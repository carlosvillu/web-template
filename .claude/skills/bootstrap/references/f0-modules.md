...
- **Entrega**: `docker-compose.prod.yml` (web standalone, worker si existe, postgres, volúmenes), `DEPLOY.md`, deploy por `git pull && docker compose up -d --build`, cron de `pg_dump` diario. TLS vía Caddy (central del VPS o del proyecto, según la infraestructura del usuario); si hay SSE, el site file necesita flush sin buffer; si hay proxy delante, resolver el trust boundary de `x-forwarded-for` AQUÍ (el rate limit de auth depende de ello). Gotcha conocido: `next start` no resuelve rutas como `next dev` — la verificación debe ejercitar `docker compose up` REAL, no solo dev.
