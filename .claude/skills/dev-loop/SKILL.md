...
- **EL ARNÉS NUNCA PUEDE SER MÁS CÓMODO QUE LA REALIDAD.** Es el anti-patrón más caro conocido de este tipo de bucle: la suite VERDE mientras la funcionalidad está rota. El detalle y las preguntas de control están en la skill `testing` (fuente de verdad); aquí, lo que el revisor tiene que hacer:
  ...
  - **Un test dice CONSERVAR una cláusula de la Verificación, pero la ejercita en OTRA CAPA** → cuando revises el test permanente que el planning exige, compara **la capa que nombra la cláusula con la capa que toca el test**. Si la Verificación dice «curl contra el servidor levantado» y el test llama al handler en proceso, **no la conserva** — y el test suele *declarar por escrito* que sí, que es lo que apaga la alarma de todos. Corolario: **una rama de config que ningún test instancia no existe** (una rama por `NODE_ENV` que solo vive en dev no la ve una suite que corre en `test`).
  - **Un assert NEGATIVO que no puede casar** → cuando revises un `not.toContain`/grep de ausencia (privacidad, secretos, fugas), pregunta si el literal PUEDE aparecer por ese canal: ¿sobrevive al truncado, al HTML-escaping, a la decodificación del camino real? ¿el canal lleva datos (control positivo en la misma pasada)? ¿cubre también las formas DERIVADAS del dato (el valor decodificado, no solo el codificado)? Un assert de ausencia que no puede casar es verde con la fuga delante. El detalle vive en la forma 8 de la skill `testing`.
...
Los incidentes más caros de este tipo de bucle no son bugs de código sino **premisas heredadas tratadas como hechos**. Observado en proyectos reales: un gotcha técnico heredado de la plantilla que era falso y solo cayó cuando un implementer lo PROBÓ con un build real en vez de obedecerlo; una premisa sobre el estado de producción («no hay usuarios/datos») que caducó sin que el bucle la re-comprobara y siguió justificando riesgos con ella; y un incidente de producción causado por sobrescribir un fichero de entorno creyendo que era config de desarrollo caducada — con la evidencia de lo contrario escrita en el propio journal. Tres reglas vinculantes:
